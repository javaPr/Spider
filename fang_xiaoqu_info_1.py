...
def task():
    while not inputq.empty():
        ...
        if xiaoqu_info[2] == "":
            xiaoqu_info[2] = u"未知"
        xiaoqu_info[0], xiaoqu_info[1], xiaoqu_info[2], xiaoqu_info[3], xiaoqu_info[4], xiaoqu_info[5], xiaoqu_info[6], \
        xiaoqu_info[7], xiaoqu_info[8] = xiaoqu_info[1], xiaoqu_info[3], xiaoqu_info[6], xiaoqu_info[7], xiaoqu_info[
            4], latitude, longtitude, xiaoqu_info[2], xiaoqu_info[5]
        outputq.put(xiaoqu_info)


all_url_list = []
temp_infp = []
f = codecs.open("fang/city.dat", 'r', 'utf8')
for each0 in f:
    ...
    if pg == 0:
        ...
        for each in list1:  # 每个小地区
            ...
            if pages == 0:
                continue
            url_list = [url[:-14] + "0_1_0_" + "{}_0_0_0/".format(str(i)) for i in range(1, pages + 1)]
            for ul in url_list:
                inputq.put(ul)
            while not inputq.empty():
                name_list = []
        ...
        d = Distribute.Distribute(8, task)
        d.execute()
        new_xiaoqu_info = []
        ...
        url_list = [url[:-14] + "0_1_0_" + "{}_0_0_0/".format(str(i)) for i in range(1, pg + 1)]
        all_url_list = []
        temp_infp = []
        for ul in url_list:
            inputq.put(ul)
