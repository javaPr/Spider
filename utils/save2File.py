...
def savePoiInMultiplyThread(dataList, filepath='data\scale_hotel_shanghai_whole_MulThread_723.txt'):
...
def save2FileMulThread(dataList, filePath=r'data\resapi.txt'):
    if saveThreadLock.acquire(True):
        with open(filePath, 'a+', encoding='utf-8') as f:
            for data in dataList:
                f.writelines(str(data.get('id', '')) + "\t")
                f.writelines(str(data.get('name', '')) + '\t')
                f.writelines(str(data.get('type', '')) + '\t')
                f.writelines(str(data.get('typecode', '')) + '\t')
                f.writelines(str(data.get('address', '')) + '\t')
                f.writelines(str(data.get('location', '')) + '\t')
                f.writelines(str(data.get('tel', '')) + '\t\n')
    saveThreadLock.release()


def saveFinalPoi(row, filepath='data\poi_final_24.txt'):
    if saveThreadLock.acquire(True):
        with open(filepath, 'a+', encoding='utf-8') as f:
